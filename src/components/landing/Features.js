...
function Features() {
  ...
  return (
    <section className="container grid grid-cols-1 xl:grid-cols-2 gap-x-10 lg:gap-x-[58px] sm:rounded-[40px] bg-[#F2FCF4] dark:bg-[#313b33] mb-28 lg:mb-[140px] px-6 xl:px-0">
      <ScrollAnimation animateOnce animateIn="fade-in-bottom">
        <div className="sm:pl-10 lg:pl-20  py-12 lg:py-[60px] text-center lg:text-left">
          ...
          <div className="grid sm:grid-cols-2 gap-10 text-center lg:text-left">
            {content.map((item, index) => (
              <div key={`${index}text`}>
                {index === 0 && (
                  <BsGrid1X2 className="mb-4 w-8 h-8 text-primary  mx-auto lg:mx-0" />
                )}
                {index === 1 && (
                  <TbUsers className="mb-4 w-8 h-8 text-primary  mx-auto lg:mx-0" />
                )}
                <h4 className="font-bold text-xl lg:text-2xl text-[#16054E] dark:text-white/80 mb-2">
                  {item.title}
                </h4>
                <p className="text-[#16054E] dark:text-white/80 text-sm max-w-[350px] mx-auto">
                  {item.text}
                </p>
              </div>
            ))}
